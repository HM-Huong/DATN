...
static void showTopScore(const int x = 64, const int y = 16) {
...
static void updateTopScore(int score) {
	if (score <= topScore[9]) {
		return;
	}
...
// extern GpvCore gray;
static void hiddenBrickBricker(int hidden) {
	bar.bypass(!hidden);
	//	gray.bypass(!hidden);
...
void controlBrickBricker(BrickBrickerCMD cmd) {
	switch (cmd) {
	case NO_CMD:
