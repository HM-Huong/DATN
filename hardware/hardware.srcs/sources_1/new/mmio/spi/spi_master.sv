...
module spi_master (
	input  logic        clk          ,
	input  logic        reset        ,
	input  logic [ 7:0] din          , //! transmit data
	input  logic [15:0] dvsr         , // 0.5*(# clk in SCK period)
	input  logic        start        ,
	input  logic        cpol         ,
	input  logic        cpha         ,
	output logic [ 7:0] dout         , //! received data
	output logic        spi_done_tick, //! asserted for one clock cycle when done
	output logic        ready        , //! ready to accept new data
	output logic        sclk         ,
	input  logic        miso         ,
	output logic        mosi
);
	...
	// by using a register, we can avoid glitch in sclk
