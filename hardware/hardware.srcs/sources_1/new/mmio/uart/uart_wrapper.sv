...
//
module uart_wrapper #(parameter  FIFO_DEPTH_BIT = 8// # addr bits of FIFO
) (
	input  logic        clk    ,
	input  logic        reset  ,
	// slot interface
	...
	// body
	// instantiate uart
	uart_core #(.DBIT(8), .SB_TICK(16), .FIFO_W(FIFO_DEPTH_BIT)) uart_unit (.*,.dvsr(dvsr_reg), .w_data(wr_data[7:0]));
